...
Rate Limiting:
  ...
  --rate-limit N         Fixed N requests per second (e.g., --rate-limit 3)
...
def search_laws(keyword: str, page: int = 1, size: int = 20,
                search_range: int = 1, search_type: int = 2,
                status_filter: list = None) -> dict:
    """Search laws. search_range: 1=title, 2=content."""
    cache_key = _cache._key("search", keyword, str(page), str(size),
                            str(search_range), str(search_type),
    ...
    resp = _request(
        "POST", f"{BASE_URL}/law-search/search/list",
        json=payload,
...
def fetch_detail(bbbs_id: str) -> dict:
    ...
    if cached:
        return cached
    try:
        resp = _request("GET", f"{BASE_URL}/law-search/search/flfgDetails", params={"bbbs": bbbs_id})
...
def get_download_url(bbbs_id: str, fmt: str = "docx") -> str:
    resp = _request(
        "GET", f"{BASE_URL}/law-search/download/pc",
...
def preview_law(bbbs_id: str):
    ...
    label_map = {"chinese": "中文数字 (如: 第一条)", "arabic": "阿拉伯数字 (如: 第1条)", "mixed": "混合"}
    print(f"Numbering: {label_map.get(detected['primary'], detected['primary'])} | "
...
def query_article(bbbs_id: str, query: str = None, grep: str = None):
    raw = fetch_detail(bbbs_id)
    content_tree = (raw or {}).get("data", {}).get("content", {})
    detected = _detect_numbering_patterns(content_tree)
    try:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=
            ...
            "  --rate-limit N         Fixed N req/s (e.g., --rate-limit 2)\n"
            "\nEnvironment:\n"
    ...
    parser.add_argument("--rate-limit", default="auto",
                        help="Rate limit mode (auto/off/fixed/adaptive/N)")
...
def main():
    ...
    try:
        ...
        if args.info:
            ...
            data = search_laws(args.search, page=args.page, size=args.size,
                               search_range=search_range, search_type=search_type,
